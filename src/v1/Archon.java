...
public class Archon extends Globals{
  ...
  static int earlyScouts_channel = 5;
  ...
  private static MapLocation[] moveLocations = new MapLocation[12];
    ...
    // The code you want your robot to perform every round should be in this loop
      ...
      try {
        ...
        BulletInfo[] nearbyBullets = rc.senseNearbyBullets(RobotType.ARCHON.bulletSightRadius);
          ...
          for (int angleIndex = 0; angleIndex < 12; ++angleIndex) {
            ...
            if (willCollide) {
              directionWeights[angleIndex] -= (10000
                  + 1000 * (myType.strideRadius + RobotType.ARCHON.bulletSightRadius - distToRobot));
              if (DEBUG) {
	...
	public static void loop() throws GameActionException{
	  ...
	  if (producedGardeners == 0){
	    if(rc.canHireGardener(Direction.getNorth())){
	      ...
	      rc.broadcast(producedGardeners_channel, 1);
	    }
	  }
	  int producedScouts = rc.readBroadcast(earlyScouts_channel);
	  while(producedScouts < 3){
	    moveDirection();
	    Clock.yield();
	    producedScouts = rc.readBroadcast(earlyScouts_channel);
	  }
		while(true){
